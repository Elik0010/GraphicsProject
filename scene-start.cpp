// VERSION: scene-start_TFpt1B-EKpt1ACDE.cpp
...
static float oldUpAndOver = 20;
static float oldSideways = 0;
...
static vec3 findMousePosition(int x, int y)
{
	...
	GLdouble project[16];                // Where The 16 Doubles Of The Projection Matrix Are To Be Stored
	glGetDoublev(GL_PROJECTION_MATRIX, project);     // Retrieve The Projection Matrix
	...
	gluUnProject( winX, winY, winZ, modelview, project, viewport, &posX, &posY, &posZ);
...
static float findDist(vec4 scene, vec3 mouse)
{
	float Xgap = scene.x - mouse.x;
	float Ygap = 0;//scene.y - mouse.y;
	float Zgap = scene.z - mouse.z;
...
static void selectNewObject()
{
		glutPostRedisplay();
        	vec3 mousePosition = findMousePosition(mouseX,mouseY);
        	float dist = 10000; //if everything is further than this, dont select anything
        	int closest = 0;
        	for(int i = 2; i < nObjects; i++)
        	{
			SceneObject s = sceneObjs[i];
			float check = findDist(s.loc, mousePosition);
        	        if(check < dist)//s.loc[0] * s.loc[0] + s.loc[1] * s.loc[1] + s.loc[2] * s.loc[2] < dist)
        	        {
        	        	closest = i;
        	        	dist = check;
        	        }
		...
		if(closest != 0)
		{	 
			toolObj = currObject = closest;//sceneObjs[closest];
		}
}
...
static void mouseClickOrScroll(int button, int state, int x, int y)
{
	...
	if(reSelect == true) selectNewObject(); deactivateTool();
...
void duplicateObject(int i)
{
	if(nObjects > 2 && i >= 2)
	{
		sceneObjs[nObjects] = sceneObjs[i]; 
...
static void changeCurrent()
{
	setToolCallbacks(NULL, mat2(400,0,0,-2),
			NULL, mat2(400,0,0,-2));
}
...
void drawMesh(SceneObject sceneObj)
{
    ...
    // *** TF: appended Rotate transformation matrices for part B

    mat4 model = Translate(sceneObj.loc) * Scale(sceneObj.scale) * RotateX(-1 * sceneObj.angles[0]) * RotateY(sceneObj.angles[1]) * RotateZ(-1 * sceneObj.angles[2]);
...
void display( void )
{
    ...
    // backwards.  You'll need to add appropriate rotations. - DONE.
 
    view = Translate(0.0, 0.0, -viewDist) * RotateX(camRotUpAndOverDeg) * RotateY(camRotSidewaysDeg);	//this needs to change for part A - DONE.
...
static void adjustAmbientAndDiffuse(vec2 ad)
{
	sceneObjs[toolObj].ambient += ad[0];
	sceneObjs[toolObj].diffuse += ad[1];
}

static void adjustSpecularAndShine(vec2 ss)
{
...
static void adjustAngleZTexscale(vec2 az_ts)
{
    ...
    sceneObjs[currObject].texScale += 20*az_ts[1];
...
static void mainmenu(int id)
{
    ...
    if (id == 63) duplicateObject(currObject);
    if (id == 64) deleteObject(currObject);
    if (id == 99) exit(0);
}
...
static void makeMenu()
{
    ...
    glutAddMenuEntry("Delete Current", 64);
    glutAddMenuEntry("Duplicate Current", 63);
    glutAddMenuEntry("Rotation/Texture Scale", 55);
    glutAddSubMenu("Material", materialMenuId);
    glutAddSubMenu("Texture",texMenuId);
    glutAddSubMenu("Ground Texture",groundMenuId);
    glutAddSubMenu("Lights",lightMenuId);
    glutAddMenuEntry("Change Current Object", 26);
