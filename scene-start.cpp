...
void display( void )
{
    ...
    view = Translate(0.0, 0.0, -viewDist) * RotateX(camRotUpAndOverDeg) * RotateY(camRotSidewaysDeg);	//this needs to change for part A
    float xDistance = viewDist * cos(camRotUpAndOverDeg) * cos(camRotSidewaysDeg);
    float yDistance = viewDist * sin(camRotUpAndOverDeg);
    float zDistance = viewDist * cos(camRotUpAndOverDeg) * sin(camRotSidewaysDeg); 
    //view = Rotate( * Translate(xDistance, yDistance, -zDistance); 
